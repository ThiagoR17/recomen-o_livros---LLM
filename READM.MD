...
Este projeto foi inicialmente criado em **Python 3.11**. Para rodar o projeto, as seguintes **dependências são requeridas**:
...
Um arquivo `requirements.txt` contendo todas as dependências do projeto é fornecido neste repositório.

Para criar seu banco de dados vetorial, você precisará criar um arquivo **`.env`** no seu diretório raiz contendo sua **chave API da OpenAI**. As instruções sobre como fazer isso fazem parte do tutorial.
